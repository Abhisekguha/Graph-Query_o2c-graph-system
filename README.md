...
![LLM](https://img.shields.io/badge/LLM-Gemini-yellow)
...
│   Gemini Pro API     │      │   (NetworkX Graph Object)    │
│   (Google AI)        │      │                              │
│   • NL Understanding │      │   • Nodes: ~133K entities    │
│   • Query Intent     │      │   • Edges: ~216K relations   │
...
**AI/LLM Integration:**
- **Google Gemini Pro** - Natural language understanding and query processing
  - 30K context window
...
|-----------|---------------|---------------|
| Customer | customerId, name, accountGroup, creditLimit | ~1,000 |
| SalesOrder | salesOrder, orderDate, netValue, currency | ~10,000 |
| SalesOrderItem | salesOrderItem, product, quantity, price | ~50,000 |
| Delivery | delivery, deliveryDate, shippingPoint | ~8,000 |
| DeliveryItem | deliveryItem, quantity, deliveryNumber | ~40,000 |
| Invoice | billingDocument, billingDate, netAmount | ~7,000 |
| InvoiceItem | billingDocumentItem, taxAmount | ~35,000 |
| JournalEntry | accountingDocument, postingDate, amount | ~6,000 |
| Payment | clearingDocument, paymentDate, amount | ~5,000 |
| Product | product, productDescription, baseUnit | ~5,000 |
| Plant | plant, plantName, country | ~50 |
| Address | addressId, street, city, postalCode | ~1,000 |
...
**Why Gemini:**
- **Free Tier**: Generous limits (60 RPM) without credit card requirement
- **Strong Reasoning**: Excellent at structured output generation and query understanding
- **Fast Response**: Low latency (<2s) for conversational queries
- **Context Window**: 30K tokens sufficient for graph context + conversation history
- **Structured Output**: Reliable JSON-style response formatting
...
- **Graph Build**: ~10-15s on startup
- **3D Rendering**: 60 FPS on modern hardware (GTX 1060+ / equivalent)
...
- **Google Gemini** - For advanced LLM capabilities and generous free tier
...
| **Backend** | FastAPI + NetworkX | <100ms query response |
| **AI** | Google Gemini Pro | Natural language understanding |
| **Visualization** | react-force-graph-3d | Physics-based layout |
| **Data Model** | MultiDiGraph | 133K nodes, 216K edges |
